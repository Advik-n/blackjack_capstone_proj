...
class BlackjackGame {
  ...
  DeckModel get deck => _deck; // Expose deck for UI
  ...
  void _handleResult() {
    ...
    switch (result!) {
      ...
      case BlackjackResult.dealerBust:
        scoreboard.wins++;
        if (scoreboard.streakType == 'win') {
          scoreboard.streak++;
        } else {
          scoreboard.streak = 1;
          scoreboard.streakType = 'win';
        }
      ...
      case BlackjackResult.playerBust:
        scoreboard.losses++;
        if (scoreboard.streakType == 'loss') {
          scoreboard.streak++;
        } else {
          scoreboard.streak = 1;
          scoreboard.streakType = 'loss';
        }
        scoreboard.balance -= currentBet.amount;
